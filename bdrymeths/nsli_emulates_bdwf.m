...
% Barnett 9/14/20; updated to repeating 1st bdry pt, 9/21/20
if nargin==0, test_nsli_emulates_bdwf; return; end

if xVals(end)~=xVals(1) || yVals(end)~=yVals(1)
  warning('last bdry pt must repeat the 1st, according to bdwf input format!');
else
  xVals = xVals(1:end-1); yVals = yVals(1:end-1);  % kill repeated pt
end
...
psi1 = 1.5e-6; psi2 = pi/5;   % incident wave dir (psi1.Z must be < O(1))
...
ns = [1e2 3e2 1e3 3e3 1e4];   % convergence study: #s of boundary nodes
                       % depends on max FN & target pt; expect 1/n^2 for NSLI
for n=ns
  t = 2*pi*(0:n-1)/n; bx = x(t); by = y(t);      % make bdry points
  % compare the two methods...
  bxrep = [bx bx(1)]; byrep = [by by(1)];        % bdwf needs 1st pt repeated!
  tic; Eb = bdwf(bxrep,byrep,[], Z, lambda, dxO, nO, psi1,psi2, deltaX,deltaY); %toc
  tic; En = nsli_emulates_bdwf(bxrep,byrep,[], Z, lambda, dxO, nO, psi1,psi2, deltaX,deltaY); %toc
  fprintf('n=%d\t max E diff:  %.3g (ampl only)   \t (%.3g including phase)\n', n, max(abs(abs(Eb(:))-abs(En(:)))), max(abs(Eb(:)-En(:))))
end

% Notes:

% 1) We know NSLI is O(1/n^2) accurate using simple trap rule weights, and
%    BDWF also O(1/n^2) via midpoint rule.
...
% why?  Turns out this is due to a single point being near bdry: bdwf suffers.
% This was debugged via reducing the largest ns then doing,
% figure; imagesc(squeeze(abs(Eb(:,:,1))-abs(En(:,:,1)))); colorbar
