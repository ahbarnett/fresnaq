function [u xigrid] = fresnap_grid(xq, yq, wq, lambdaz, ximax, ngrid, tol, verb)
% FRESNAP_GRID  fast scalar Fresnel aperture diffraction, square target grid
%
% [u xigrid] = fresnap_grid(xq, yq, wq, lambdaz, ximax, ngrid, tol)
...
%  in the xi,eta plane.
%
% ... = fresnap_grid(xq, yq, wq, lambdaz, ximax, ngrid, tol, verb)
...
%  faster than fresnap_pts for large grids.
%
%  Example: see test_fresnap_grid
%
...
u = kirchfac * u;
if verb, fprintf('fresnap_grid: N=%d quadr, M=%d targs, %.3g s\n',numel(xq),numel(u),toc(t0)), end


%%%%
function test_fresnap_grid
...
% validate against slower fresnap_pts for all targets...
upts = fresnap_pts(xq, yq, wq, lambdaz, xi(:), eta(:), tol);
fprintf('max abs err fresnap_pts vs grid: %.3g\n',norm(upts-u(:),inf))
